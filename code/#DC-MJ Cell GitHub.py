...
plt.xlabel('Bandgap Energy (eV)', fontsize=18, fontweight='bold')
plt.ylabel('Efficiency / Loss Components (%)', fontsize=18, fontweight='bold')
plt.xticks(fontsize=14) 
plt.yticks(fontsize=14)
plt.title('Solar Cell Efficiency Breakdown and Loss Mechanisms', fontsize=18, fontweight='bold', pad=15)
legend = plt.legend(loc='upper right', frameon=True, framealpha=0.95, facecolor='#f8f8f8', edgecolor='#404040',fontsize = 14)
...
plt.xticks(major_ticks + list(minor_ticks), fontsize = 14)
plt.tick_params(axis='x', which='major', length=8)
plt.tick_params(axis='x', which='minor', length=4)
plt.xlabel('Bandgap (eV)', fontsize = 18)
plt.ylabel('Number of MJ Cells', fontsize = 18)
plt.title('Optimal Bandgap Distribution vs MJ Count', fontsize = 18)
plt.yticks(range(1, 13), fontsize = 14)
plt.grid(True, axis='x', linestyle='--', alpha=0.5)
plt.legend(title='MJ Cell Count', bbox_to_anchor=(1.05, 1), loc='upper left', fontsize = 14)
plt.tight_layout()
plt.show()
...
im = plt.imshow(eff_map, origin='lower', aspect='auto', cmap='inferno')
cbar = plt.colorbar(im)
cbar.set_label('Efficiency (%)', fontsize=18)  # fontsize for label
cbar.ax.tick_params(labelsize=14)              # fontsize for tick labels
plt.xlabel('DC Layer Count', fontsize = 18)
plt.ylabel('MJ Cell Count', fontsize = 18)
plt.title('Efficiency Heatmap: MJ vs DC', fontsize = 18)
plt.xticks(ticks=np.arange(pivot.columns.size), labels=pivot.columns, fontsize = 14)
plt.yticks(ticks=np.arange(pivot.index.size), labels=pivot.index, fontsize = 14)
for dc in fix_dc:
...
plt.xlabel('MJ Cell Count', fontsize = 18)
plt.ylabel('Efficiency (%)', fontsize = 18)
plt.title('Efficiency vs MJ Cells @ Fixed DC Layers', fontsize = 18)
plt.xticks(fontsize=14)  
plt.yticks(fontsize=14)
...
plt.tight_layout()
plt.show()
#Figure 4b, Efficiency vs DC for fixed MJ layers
plt.figure(figsize=(8,6))
for mj in fix_mj:
    if mj in pivot.index:
...
plt.xlabel('DC Layer Count', fontsize = 18)
plt.ylabel('Efficiency (%)', fontsize = 18)
plt.title('Efficiency vs DC Layers @ Fixed MJ Cells', fontsize = 18)
plt.xticks(fontsize=14)  
plt.yticks(fontsize=14)
...
plt.tight_layout()
plt.show()
#Figure 5 Irradiance vs Bandgap for fixed MJ=12, varying DC
efficiencies = np.array([compute_efficiency(Eg, irradiance, wavelength) for Eg in bandgap])
...
axs[0].set_ylabel('Irradiance (W/m²/eV)', fontsize = 14)
axs[0].set_xlabel('Energy (eV)', fontsize = 14)
axs[0].tick_params(axis='x', labelsize=12)
axs[0].tick_params(axis='y', labelsize=12)
axs[0].set_title('Down-Converted Spectrum (vs. Energy)', fontsize = 14)
axs[0].grid(True)
...
axs[1].set_xlabel('Bandgap (eV)', fontsize = 14)
axs[1].set_ylabel('Efficiency (%)', fontsize = 14)
axs[1].tick_params(axis='x', labelsize=12)
axs[1].tick_params(axis='y', labelsize=12)
axs[1].set_title('SQ Limit (Down-Converted Spectrum)', fontsize = 14)
axs[1].grid(True)
plt.tight_layout()
plt.show()
...
ax.tick_params(axis='x', which='both', length=0, labelsize = 18)
ax.tick_params(axis='y', labelsize=18) 
ax.set_xticks([])         
ax.set_ylabel('Irradiance (W/m²/eV)', fontsize = 22)
ax.set_title('Shifted AM1.5G Spectra vs Energy for Varying DC Layers', fontsize = 22)
...
plt.ylabel('Irradiance (W/m²/eV)', fontsize = 18)
plt.xlabel('Energy (eV)', fontsize = 18)
plt.xticks(fontsize=14) 
plt.yticks(fontsize=14)
plt.title('Modified Spectrum After DC and UC Processing', fontsize = 18)
plt.grid(True)
plt.xlim(0, 4)
plt.ylim(0, None)
plt.tight_layout()
...
ax1 = fig.add_subplot(gs[:, 0])  # use all rows
...
ax1.set_ylabel('Power Density (W/m²)', fontsize=18)
ax1.set_xlabel('Subcell Index', fontsize=18)
ax1.set_title('Power Generation by Subcell', fontsize=18)
ax1.set_xticks(x)
